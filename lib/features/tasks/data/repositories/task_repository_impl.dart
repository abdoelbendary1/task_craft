...
import 'package:task_craft/core/enum/https_mehtods.dart';
import 'package:task_craft/core/errors/failures.dart';
import 'package:task_craft/core/errors/app_exceptions.dart'; // 🟢 Your custom exceptions
import 'package:task_craft/core/network/interceptor/api_endpoints.dart';
import 'package:task_craft/core/network/success_repsonse.dart';
import 'package:task_craft/core/services/sync/sync_manager.dart';
import 'package:task_craft/features/tasks/data/model/task_model.dart';
...
class TaskRepositoryImpl implements TaskRepository {
  ...
  Stream<List<TaskEntity>> watchTasksStream(String projectId) {
    return _localDataSource
        .watchCachedTasks(projectId)
        .map((models) => models.map((m) => m.toEntity()).toList());
  }

  @override
  Future<Either<Failure, Success<List<TaskEntity>>>> refreshTasksCache(
    String projectId,
  ) async {
    try {
      final connection = await _connectivity.checkConnectivity();
  ...
  @override
  Future<Either<Failure, Success<TaskEntity>>> createTask({
    required TaskEntity task,
    required String projectId,
  }) async {
    final clientSideId = _uuid.v4();
    final model = task.toModel().copyWith(
      id: clientSideId,
      createdAt: DateTime.now(),
      projectId: projectId,
    );

    // 1. Single Source of Truth: update local UI state immediately
    await _localDataSource.cacheSingleTask(model);

    try {
      // 2. Transmit changes to cloud database
      final response = await _remoteDataSource.createRemoteTask(
        task: model,
        projectId: projectId,
      );
      return Right(
      ...
      // 3. Graceful handoff to background worker on network exception
      ...
      // 4. Server configuration or payload rejection fallback: Revert local state modifications
      await _localDataSource.deleteCachedTask(model.id);
      return Left(ServerFailure(message: e.toString()));
    }
  }

  @override
  Future<Either<Failure, Success<TaskEntity>>> updateTaskStatus(
    String taskId,
    int statusId,
  ) async {
    // 1. Grab current snapshot from memory box
    final cachedTask = await _localDataSource.getCachedTaskById(taskId);
    if (cachedTask == null) {
      return const Left(
        CacheFailure(message: "Task data could not be located in cache."),
      );
    }

    // 2. Optimistic UI mutation pass
    final updatedModel = cachedTask.copyWith(statusId: statusId);
    await _localDataSource.cacheSingleTask(updatedModel);

    try {
      // 3. Sync state alteration with live database
      final response = await _remoteDataSource.updateRemoteTaskStatus(
        taskId,
        statusId,
      );

      // Cache server response validation snapshot
      await _localDataSource.cacheSingleTask(response);
      ...
      // 4. Handle structural validation or parameter rule failures (e.g. Server Rejected data)
      if (e is! NetworkException) {
        // Explicit structural rollback: restore original task model state
        await _localDataSource.cacheSingleTask(cachedTask);
        return Left(ServerFailure(message: e.toString()));
      }

      // 5. Normal offline mode network fallback: append action execution sequence to SyncManager
      await _syncManager.addToActionQueue(
        endpoint: ApiEndpoints.updateOrDeleteTask(taskId),
        method: HttpMethod.PATCH.name,
        payload: {'status_id': statusId},
  ...
  @override
  Future<Either<Failure, Success>> deleteTask(String taskId) async {
    // 1. Instant local removal from source box
    await _localDataSource.deleteCachedTask(taskId);

    try {
      // 2. Deliver remote drop notification payload to remote server
      await _remoteDataSource.deleteRemoteTask(taskId);
      return Right(
        Success(
          data: unit,
      ...
      // 3. Fallback tracking logic when connection is absent
      if (e is NetworkException) {
        await _syncManager.addToActionQueue(
          endpoint: ApiEndpoints.updateOrDeleteTask(taskId),
        ...
        return Right(
          Success(
            data: unit,
            message: "Removed locally (Will sync deletion later)",
            statusCode: 202,
          ),
        );
      }
